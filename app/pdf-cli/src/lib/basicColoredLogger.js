...
exports.init = function (
  ...
  exports.config.colors = Object.assign({}, exports.defaultSettings.colors, colors); // handle no setting & partial setting
  exports.config.quiet = quiet;

  if (!quiet) {
    // print logger config message
    const isFiltered = Object.values(exports.config.levelFilter).indexOf(false) !== -1; // are any levelFilters set to false?
    const hasColors = Object.values(exports.config.colors).filter((x) => x).length > 0; // are any colors defined?
    let message = `basicColoredLogger ${host} isFiltered=${isFiltered}`;
...
exports.shutdown = function () {
  if (!exports.config.quiet) {
    console.log("shutdown basicColoredLogger");
  }
};
...
function logger(level, filter, colors, consolelogger, ...args) {
  if (filter) {
    ...
    if (colors) {
      consolelogger(colors(new Date().toISOString(), level, exports.config.host, ...args));
    } else {
      consolelogger(new Date().toISOString(), level, exports.config.host, ...args);
    }
  }
}
